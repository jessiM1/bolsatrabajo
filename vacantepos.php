...
$conexion = new mysqli("localhost", "root", "", "bolsatrabajo");
...
// Obtener las vacantes publicadas por la empresa (ejemplo: id_empresa = 1)
$id_empresa = 1; // Cambiar según el usuario logueado
$sql = "SELECT * FROM vacantes WHERE id_empresa = ?";
...
$sql = "SELECT * FROM vacantes LIMIT $inicio, $por_pagina";
$resultado = $conexion->query($sql);

$total_vacantes = $conexion->query("SELECT COUNT(*) as total FROM vacantes")->fetch_assoc()['total'];
$total_paginas = ceil($total_vacantes / $por_pagina);
?>
         ...
         <div class="container">
            <div class="row">
               <div class="col-md-12">
                ...
                <?php if ($vacante = $resultado->fetch_assoc()): ?>
                    <form action="" method="POST" class="form-horizontal">
                       <fieldset>
               

                                <label for="puesto">Puesto:</label>
                                <input type="text" id="puesto" value="<?= htmlspecialchars($vacante['puesto']) ?>"  disabled>
                            
                            
                                <label for="salario">Salario:</label>
                                <input type="text" id="salario" value="<?= htmlspecialchars($vacante['salario']) ?>"  disabled>
                           
                                <label for="horario">Horario:</label>
                                <input type="text" id="horario" value="<?= htmlspecialchars($vacante['horario']) ?>"  disabled>
                           
                          
                                <label for="habilidades">Habilidades:</label>
                                <textarea id="habilidades"  disabled><?= htmlspecialchars($vacante['habilidades']) ?></textarea>
                           
                           
                                <label for="prestaciones">Prestaciones:</label>
                                <textarea id="prestaciones"  disabled><?= htmlspecialchars($vacante['prestaciones']) ?></textarea>
                           
                                <label for="experiencia">Experiencia:</label>
                                <input type="text" id="experiencia" value="<?= htmlspecialchars($vacante['experiencia'] . " " . $vacante['tiempo_experiencia']) ?>"  disabled>
                           
                            
                                <a href="postulantes.php?id_vacante=<?= $vacante['id_vacante'] ?>" class="read_more" style="display: block; margin: 0 auto;">Ver Postulantes</a>
                ...
                <div class="pagination">
                    ...
                    <a href="?pagina=<?= $pagina_actual - 1 ?>"><img src="guardadas/anterior.png" width="50" height="50"</a><br>Anterior</p>
                    <p>
                      <?php endif; ?>
                      
                      <?php if ($pagina_actual < $total_paginas): ?>
                      <a href="?pagina=<?= $pagina_actual + 1 ?>"><img src="guardadas/siguiente.png" width="50"  height="50"</a>
                    <br>Siguiente</p>
                    <?php endif; ?>
        </div>
</div>
