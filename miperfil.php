...
if (!isset($_SESSION['correo']) || !isset($_SESSION['buscador_id'])) {
    header("Location: login.php");
    exit();
}

$correo = $_SESSION['correo'];
$id_buscador = $_SESSION['buscador_id'];

// Obtener información del buscador
$sql = "SELECT nombre, apaterno, amaterno, direccion, fn, edad, correo, telefono, foto_perfil, foto_portada FROM buscadores WHERE id_buscador = $id_buscador";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['actualizar_datos'])) {
    ...
    $apaterno = mysqli_real_escape_string($conn, $_POST['apaterno']);
    $amaterno = mysqli_real_escape_string($conn, $_POST['amaterno']);
    ...
    $sql_update = "UPDATE buscadores SET 
        nombre = '$nombre', 
        apaterno = '$apaterno', 
        amaterno = '$amaterno', 
        direccion = '$direccion', 
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['actualizar_perfil'])) {
    ...
    if (isset($_FILES['profile-upload']) && $_FILES['profile-upload']['error'] == 0) {
        ...
        if (move_uploaded_file($_FILES["profile-upload"]["tmp_name"], $profile_target_file)) {
            ...
            $sql_update_perfil = "UPDATE buscadores SET foto_perfil = '$foto_perfil' WHERE id_buscador = $id_buscador";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['actualizar_portada'])) {
    ...
    if (isset($_FILES['cover-upload']) && $_FILES['cover-upload']['error'] == 0) {
        ...
        if (move_uploaded_file($_FILES["cover-upload"]["tmp_name"], $cover_target_file)) {
            ...
            $sql_update_portada = "UPDATE buscadores SET foto_portada = '$foto_portada' WHERE id_buscador = $id_buscador";
...
<?php 
$foto_perfil = $buscador['foto_perfil'] ? $buscador['foto_perfil'] : 'http://localhost/Avo/ESCUELA/basco-html/images/perfil.png';
?>
<div class="pro-img">
...
<!-- Mostrar foto de portada actual -->
                    ...
                    <h2><?= htmlspecialchars($buscador['nombre']); ?> </h2>
                  
                 <br>  
        <p>Apellido Paterno: <?= htmlspecialchars($buscador['apaterno']); ?></p>
        <br>
        <p>Apellido Materno: <?= htmlspecialchars($buscador['amaterno']); ?></p>
        <br>
        <p>Dirección: <?= htmlspecialchars($buscador['direccion']); ?></p>
        <br>
        <p>Fecha de Nacimiento: <?= htmlspecialchars($buscador['fn']); ?></p>
        <br>
        <p>Edad: <?= htmlspecialchars($buscador['edad']); ?></p>
        <br>
        <p>Correo: <?= htmlspecialchars($buscador['correo']); ?></p>
        <br>
        <p>Teléfono: <?= htmlspecialchars($buscador['telefono']); ?></p>
               ...
               <div class="parent-container">
                ...
                <div class="modal-body">
                    ...
                    <form method="POST" action="">
                            ...
                            <input type="text"  name="nombre" value="<?= htmlspecialchars($buscador['nombre']); ?>" required>
                      
                            <label for="apaterno">Apellido Paterno</label>
                            <input type="text"  name="apaterno" value="<?= htmlspecialchars($buscador['apaterno']); ?>" required>
                        
                       
                            <label for="amaterno" >Apellido Materno</label>
                            <input type="text"  name="amaterno" value="<?= htmlspecialchars($buscador['amaterno']); ?>" required>
                       
                       
                            <label for="direccion">Dirección</label>
                            <input type="text"  name="direccion" value="<?= htmlspecialchars($buscador['direccion']); ?>" required>
                     
                        
                            <label for="fn" >Fecha de Nacimiento</label>
                            <input type="date" name="fn" value="<?= htmlspecialchars($buscador['fn']); ?>" required>
                        
                       
                            <label for="edad" >Edad</label>
                            <input type="number"  name="edad" < readonly value="<?= htmlspecialchars($buscador['edad']); ?>" required>
                        
                            <label for="correo" >Correo</label>
                            <input type="email"  name="correo" value="<?= htmlspecialchars($buscador['correo']); ?>" required>
                       
                            <label for="telefono">Teléfono</label>
                            <input type="text"  name="telefono" value="<?= htmlspecialchars($buscador['telefono']); ?>" required>
